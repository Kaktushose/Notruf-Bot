package io.github.kaktushose.notruf.embeds;
...
import static io.github.kaktushose.notruf.moderation.modlog.ModlogCommand.ModlogContext;
...
public class EmbedHelpers {
    ...
    public static MessageEmbed getModlogEmbedHeader(EmbedCache embedCache, ModlogContext context) {
        ...
        if (context.member() == null) {
                   embed.injectValue("joinedAt", "?DEL?");
        } else {
            embed.injectValue("joinedAt", context.member().getTimeJoined().getLong(ChronoField.INSTANT_SECONDS));
        }
